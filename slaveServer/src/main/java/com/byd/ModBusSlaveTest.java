...
public class ModBusSlaveTest {
    ...
    try {
        ...
        // 创建一个从机
        final ModbusSlave slave = ModbusSlaveFactory.createModbusSlaveTCP(tcpParameters);
        // 设置控制台输出主机和从机命令交互日志
        ...
        ModbusHoldingRegisters hr = new ModbusHoldingRegisters(1111);
        // 修改数值寄存器对应位置的值，第一个参数代表寄存器地址，第二个代表修改的数值
        hr.set(0, 65535);
        hr.set(1110, 65534);
        // 设置从机寄存器的01和02功能码对应的位寄存器，即只有false和true值（或0和1）
        ModbusCoils mc = new ModbusCoils(30000);
        // 设置对应位寄存器地址的位值
        mc.set(12800, true);
        // 为从机设置04功能码对应的数值寄存器
